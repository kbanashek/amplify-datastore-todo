...
import { createMockTaskHistory } from '../../__tests__/__mocks__/DataStore.mock';
...
describe('TaskHistoryService', () => {
  ...
  describe('createTaskHistory', () => {
    ...
    it('should throw error on create failure', async () => {
      ...
      await expect(TaskHistoryService.createTaskHistory(input)).rejects.toThrow('Create failed');
    });
  });
  ...
  describe('updateTaskHistory', () => {
    it('should update a task history successfully', async () => {
      const originalHistory = createMockTaskHistory({ id: 'test-id', status: 'OPEN' });
      const updatedHistory = createMockTaskHistory({ id: 'test-id', status: 'COMPLETED' });
      ...
      const result = await TaskHistoryService.updateTaskHistory('test-id', { status: 'COMPLETED' });
    ...
    it('should throw error if task history not found', async () => {
      ...
      await expect(
        TaskHistoryService.updateTaskHistory('non-existent', { status: 'COMPLETED' })
  ...
  describe('deleteTaskHistory', () => {
    ...
    it('should throw error if task history not found', async () => {
      ...
      await expect(TaskHistoryService.deleteTaskHistory('non-existent')).rejects.toThrow(
        'TaskHistory with id non-existent not found'
      );
    });
  });
  ...
  describe('clearDataStore', () => {
    it('should clear the DataStore', async () => {
      (DataStore.clear as jest.Mock).mockResolvedValue(undefined);

      await TaskHistoryService.clearDataStore();
