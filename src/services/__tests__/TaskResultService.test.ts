...
import { createMockTaskResult } from '../../__tests__/__mocks__/DataStore.mock';
...
describe('TaskResultService', () => {
  ...
  describe('createTaskResult', () => {
    ...
    it('should throw error on create failure', async () => {
      ...
      await expect(TaskResultService.createTaskResult(input)).rejects.toThrow('Create failed');
    });
  });
  ...
  describe('updateTaskResult', () => {
    it('should update a task result successfully', async () => {
      const originalResult = createMockTaskResult({ id: 'test-id', status: 'OPEN' });
      const updatedResult = createMockTaskResult({ id: 'test-id', status: 'COMPLETED' });

      (DataStore.query as jest.Mock).mockResolvedValue(originalResult);
      (DataStore.save as jest.Mock).mockResolvedValue(updatedResult);

      const result = await TaskResultService.updateTaskResult('test-id', { status: 'COMPLETED' });
    ...
    it('should throw error if task result not found', async () => {
      ...
      await expect(
        TaskResultService.updateTaskResult('non-existent', { status: 'COMPLETED' })
  ...
  describe('deleteTaskResult', () => {
    ...
    it('should throw error if task result not found', async () => {
      ...
      await expect(TaskResultService.deleteTaskResult('non-existent')).rejects.toThrow(
        'TaskResult with id non-existent not found'
      );
    });
  });
  ...
  describe('clearDataStore', () => {
    it('should clear the DataStore', async () => {
      ...
      await TaskResultService.clearDataStore();
