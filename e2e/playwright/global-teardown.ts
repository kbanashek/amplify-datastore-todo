...
async function globalTeardown() {
  ...
  if (!shouldCleanup) {
    ...
    console.log(
      "💡 [E2E Teardown] Test data remains in database for inspection"
    );
    return;
  }

  console.log(
    "🧹 [E2E Teardown] Cleaning up test data only (dev data untouched)..."
  );
  console.log(
    "🔍 [E2E Teardown] Only deleting items with E2E_TEST_MARKER prefix"
  );
  ...
  try {
    ...
    await page.evaluate(async () => {
      try {
        ...
        const { Task, Activity, TaskAnswer, TaskResult, TaskHistory } =
          Models as any;
    ...
    console.log(
      "✅ [E2E Teardown] Teardown complete - test data cleaned up, dev data untouched"
    );
  } catch (error) {
