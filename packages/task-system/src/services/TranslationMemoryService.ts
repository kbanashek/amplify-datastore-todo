...
export class TranslationMemoryService {
  ...
  /**
   * Synchronously check in-memory cache only (seed + runtime memory)
   * Returns null if not found in memory (doesn't check AsyncStorage)
   */
  static getTranslationSync(
    ...
    return fromMemory ? fromMemory.translatedText : null;
  }
  ...
  static async getTranslation(
    ...
    // First check in-memory cache synchronously
    const fromMemory = this.getTranslationSync(
      sourceText,
      sourceLanguage,
      targetLanguage
    );
