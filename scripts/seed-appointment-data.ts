...
export async function seedAppointmentData(): Promise<AppointmentData> {
  try {
    ...
    log('Seeding complete!', {
      ...
      telehealthCount: appointments.filter(apt => apt.appointmentType === AppointmentType.TELEVISIT).length,
      onsiteCount: appointments.filter(apt => apt.appointmentType === AppointmentType.ONSITE).length,
    });
