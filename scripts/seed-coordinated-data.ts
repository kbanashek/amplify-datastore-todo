...
import { Appointment, AppointmentData, AppointmentType, AppointmentStatus } from '../src/types/Appointment';
...
export async function seedCoordinatedData(): Promise<CoordinatedSeedResult> {
  ...
  try {
    ...
    const activities = [multiPageHealth, healthSurvey, painAssessment, medicalHistory];
    ...
    for (const appointment of appointments) {
      ...
      if (appointment.status === AppointmentStatus.SCHEDULED) {
        const appointmentTasks = await createAppointmentWithTasks(appointment, activities);
        linkedTasks.push(...appointmentTasks);
        
        // Track relationships
        const appointmentDate = new Date(appointment.startAt);
        const preVisitTasks = appointmentTasks.filter((t: any, i: number) => i === 0);
        const visitDayTasks = appointmentTasks.filter((t: any, i: number) => i === 1 || i === 2);
        const postVisitTasks = appointmentTasks.filter((t: any, i: number) => i === 3 || i === 4);
    ...
    log('========================================');
    log('Summary:', summary);
    log('Relationships:', relationships.map((r) => ({
