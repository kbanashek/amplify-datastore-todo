...
const { SeededDataCleanupService } = require("../src/services/SeededDataCleanupService");
...
async function waitForSync(durationMs = 5000) {
  console.log(`[ResetAndReseed] Waiting ${durationMs}ms for sync to complete...`);
...
async function resetAndReseed() {
  try {
    ...
    console.log("[ResetAndReseed] Nuclear delete complete:", deleteResult.deleted);
    
    // Step 3: Wait for deletions to sync
    console.log("[ResetAndReseed] Step 2: Waiting for deletions to sync across devices (10 seconds)...");
    ...
    if (afterDelete.taskCount > 0 || afterDelete.activityCount > 0) {
      console.log("[ResetAndReseed] ⚠️  Warning: Some data still exists after delete:", afterDelete);
    } else {
    ...
    console.log("[ResetAndReseed] Step 5: Waiting for new data to sync across devices (10 seconds)...");
    ...
    console.log("🔍 Look for device-specific logs to verify sync across devices");
