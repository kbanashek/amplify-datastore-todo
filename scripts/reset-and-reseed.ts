...
async function waitForSync(durationMs: number = 5000): Promise<void> {
  logWithDevice("ResetAndReseed", `Waiting ${durationMs}ms for sync to complete...`);
...
async function resetAndReseed(): Promise<void> {
  try {
    ...
    logWithDevice("ResetAndReseed", "Step 2: Waiting for deletions to sync across devices...");
    ...
    if (afterDelete.taskCount > 0 || afterDelete.activityCount > 0) {
      logWithDevice("ResetAndReseed", "⚠️  Warning: Some data still exists after delete", afterDelete);
    } else {
    ...
    logWithDevice("ResetAndReseed", "Step 5: Waiting for new data to sync across devices...");
    ...
    if (afterReseed.taskCount === 0 && afterReseed.activityCount === 0) {
      logErrorWithDevice("ResetAndReseed", "❌ ERROR: No data found after reseed!");
    ...
    logWithDevice("ResetAndReseed", "📱 All devices should now show the same data");
    logWithDevice("ResetAndReseed", "💡 Check iOS, Android, and Web - they should all be synced");
    
  } catch (error) {
    logErrorWithDevice("ResetAndReseed", "❌ Error during reset and reseed", error);
    throw error;
  } finally {
...
resetAndReseed()
  .then(() => {
    ...
    console.log("📱 Check all devices (iOS, Android, Web) - they should show the same data");
    process.exit(0);
  })
  .catch((error) => {
