...
export async function createTasksForActivities(activities: any[]) {
  ...
  log('Creating tasks for activities', {
    ...
    activities: activities.map((a) => ({ id: a.id, pk: a.pk, title: a.title || a.name })),
    daysToCreate: 6,
  });
...
export async function seedQuestionData() {
  ...
  try {
    ...
    const allActivities = [multiPageHealth, healthSurvey, painAssessment, medicalHistory];
    ...
    for (let dayOffset = 0; dayOffset < 3; dayOffset++) {
      ...
      const existingMultiPageTask = tasks.find(
        (t) => t.entityId === multiPageHealth.pk && 
